 ...
 */
package com.jacarrichan.generator.config;

import static com.jacarrichan.generator.internal.util.StringUtility.composeFullyQualifiedTableName;
...
import static com.jacarrichan.generator.internal.util.messages.Messages.getString;
...
import com.jacarrichan.generator.api.CommentGenerator;
...
import com.jacarrichan.generator.api.JavaFormatter;
import com.jacarrichan.generator.api.JavaTypeResolver;
import com.jacarrichan.generator.api.Plugin;
...
import com.jacarrichan.generator.internal.PluginAggregator;
import com.jacarrichan.generator.internal.db.ConnectionFactory;
import com.jacarrichan.generator.internal.db.DatabaseIntrospector;
...
public class Context extends PropertyHolder {
	...
	public XmlElement toXmlElement() {
		...
		if (javaClientGeneratorConfiguration != null) {
			xmlElement.addElement(javaClientGeneratorConfiguration
					.toXmlElement());
		}
	...
	private List<Controller> introspectedControllers;
	...
	public void introspectTables(ProgressCallback callback,
		...
		introspectedControllers=new ArrayList<Controller>();
		...
		try {
			...
			for (TableConfiguration tc : tableConfigurations) {
				String tableName = composeFullyQualifiedTableName(
			...
			PreparedStatement ps = connection
					.prepareStatement("select menu_url from system_menu");
			ps.execute();
			ResultSet rs = ps.getResultSet();
			List<Controller> controllers=databaseIntrospector.introspectController(rs);
			if (controllers != null) {
				introspectedControllers.addAll(controllers);
			}
			// /=================
	...
	public void generateFiles(ProgressCallback callback,
			...
			throws InterruptedException {
				...
				File templateFolder=new File(this.getClass().getResource("/").getPath()
						+ "/templates");
				getGeneratedFtlFiles(templateFolder);
	}
	...
	final static List<String> scanAllFiles(File dir) throws Exception {
		List<String> allFiles = new ArrayList<String>();
		File[] fs = dir.listFiles();
		for (int i = 0; i < fs.length; i++) {
			File file = fs[i];
			allFiles.add(FileHelper.getRelativePath(dir, file));
			if (file.isDirectory()) {
				allFiles.addAll(scanAllFiles(file));
			}
		}
		return allFiles;
	}
	public List<GeneratedJavaFile> getGeneratedFtlFiles(File templateFolder) {
		// 获取所有的template
		try {
			List<String> templates = scanAllFiles(templateFolder);
			// TODO
			...
			for (String templatePath : templates) {
				for (Controller controller : introspectedControllers) {
					Template template = config.getTemplate(templatePath, "UTF-8");
					template.process(controller, out);
					out.flush();
				}
			}
			out.close();
